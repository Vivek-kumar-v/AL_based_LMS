...
// ─── Voice presets ────────────────────────────────────────────────────────────
const VOICE_PRESETS = [
  { id: "default", label: "Default",     emoji: "🎙️", color: "teal",   rate: 0.95, pitch: 1.0, langHint: ""   },
  { id: "slow",    label: "Slow & Clear",emoji: "🐢", color: "blue",   rate: 0.65, pitch: 0.95,langHint: "en" },
  { id: "fast",    label: "Fast",        emoji: "⚡", color: "orange", rate: 1.5,  pitch: 1.0, langHint: "en" },
  { id: "deep",    label: "Deep",        emoji: "🎸", color: "purple", rate: 0.85, pitch: 0.5, langHint: "en" },
  { id: "high",    label: "High Pitch",  emoji: "🎵", color: "pink",   rate: 1.0,  pitch: 2.0, langHint: "en" },
  { id: "robot",   label: "Robot",       emoji: "🤖", color: "gray",   rate: 1.1,  pitch: 0.1, langHint: ""   },
];
...
// ─── Split plain text into word tokens keeping their char offsets ─────────────
...
// ─── ReadAloudText: renders plain text with per-word highlight ────────────────
const ReadAloudText = ({ plainText, activeCharIndex, activeCharLength, isActive }) => {
  const tokens = useMemo(() => tokenize(plainText), [plainText]);
  ...
  if (!isActive) {
    // when not speaking just render plain
    return (
    ...
    <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">
      {tokens.map((tok, i) => {
        const isWord =
          ...
          tok.start >= activeCharIndex &&
          tok.start < activeCharIndex + (activeCharLength || 1);

        return (
          <React.Fragment key={i}>
            <span
              ref={isWord ? activeRef : null}
              style={
                isWord
                  ? {
...
const DocumentCard = ({ doc, onDelete, onOCRDone }) => {
  ...
  const [activeCharLength, setActiveCharLength] = useState(null);
  const [showReadAlong, setShowReadAlong]       = useState(true);

  const utteranceRef  = useRef(null);
  const plainTextRef  = useRef("");   // cached stripped text for the utterance
  ...
  const buildUtterance = (text) => {
    ...
    if (selectedVoiceIdx !== null && voices[selectedVoiceIdx]) {
      ...
      const match = voices.find((v) => v.lang.startsWith(activePreset.langHint));
  ...
  const handleSpeak = () => {
    ...
    plainTextRef.current = clean;
    const utterance = buildUtterance(clean);

    utterance.onstart = () => {
      setIsSpeaking(true); setIsPaused(false);
      setActiveCharIndex(0); setActiveCharLength(1);
    };
    ...
    utterance.onend = () => {
      ...
      setActiveCharIndex(null); setActiveCharLength(null);
  ...
  const handleDelete = async () => {
    ...
    try { await deleteDocumentApi(doc._id); onDelete(doc._id); }
  ...
  const statusBadge = () => {
    if (status === "processed") return <span className="px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700 border border-green-200">Processed ✅</span>;
    if (status === "failed")    return <span className="px-3 py-1 rounded-full text-xs font-bold bg-red-100 text-red-700 border border-red-200">Failed ❌</span>;
  ...
  const plainForDisplay = useMemo(
    () => stripMarkdown(llmText || "No explanation available."),
    [llmText]
  );
  ...
  return (
    ...
    >
        ...
        <div className="space-y-1">
          ...
          {doc?.uploadedBy && (
            <div className="flex items-center gap-2 mt-2">
              <img src={doc.uploadedBy.avatar || `https://ui-avatars.com/api/?name=${doc.uploadedBy.fullName || "User"}`}
                alt="uploader" className="w-7 h-7 rounded-full object-cover border border-gray-300" />
      ...
      <div className="mt-5 flex gap-3 items-center flex-wrap">
        ...
        {isProcessed && <span className="px-4 py-2 rounded-2xl bg-green-50 text-green-700 font-bold border border-green-200">OCR Done ✅</span>}
        {isProcessed && (
            ...
            <div className="rounded-3xl border border-gray-200 bg-gradient-to-br from-gray-50 to-white p-4">
                    ...
                    {VOICE_PRESETS.map((p) => (
                      <button key={p.id} onClick={() => applyPreset(p)}
                        className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition ${
                          selectedPreset === p.id
                            ? `${COLOR[p.color].badge} ring-2 ${COLOR[p.color].ring}`
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                          <div>
                            <label className="text-xs font-bold text-gray-600 flex justify-between mb-1">
                              <span>⚡ Speed</span><span className="text-gray-400">{speed.toFixed(2)}×</span>
                            </label>
                          ...
                          <div>
                            <label className="text-xs font-bold text-gray-600 flex justify-between mb-1">
                              <span>🎵 Pitch</span><span className="text-gray-400">{pitch.toFixed(2)}</span>
                            </label>
              ...
              {ttsSupported && (
                <div className="flex items-center gap-2 flex-wrap mb-3">
                  ...
                  {/* read-along toggle */}
                  <button
                    onClick={() => setShowReadAlong((p) => !p)}
                    className={`px-3 py-2 rounded-2xl text-xs font-bold border transition ${
                      showReadAlong
                        ? "bg-violet-50 text-violet-700 border-violet-200 hover:bg-violet-100"
                        : "bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100"
                    }`}
                    title="Toggle word-by-word highlight view"
                  >
                    {showReadAlong ? "✨ Read-Along ON" : "✨ Read-Along OFF"}
                  </button>
                  ...
                  <span className={`text-xs font-bold px-3 py-1.5 rounded-full border ${COLOR[activePreset.color].badge}`}>
                    ...
                    {selectedVoiceIdx !== null && voices[selectedVoiceIdx] ? ` · ${voices[selectedVoiceIdx].name.split(" ")[0]}` : ""}
                  </span>
                </div>
              )}
              ...
              <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
                ...
                <div className="flex border-b border-gray-100">
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-bold transition ${!showReadAlong ? "bg-white text-gray-800 border-b-2 border-purple-500" : "bg-gray-50 text-gray-400 hover:text-gray-600"}`}
                  >
                    📄 Formatted
                  </button>
                  <button
                    onClick={() => setShowReadAlong(true)}
                    className={`flex-1 py-2 text-xs font-bold transition ${showReadAlong ? "bg-white text-gray-800 border-b-2 border-violet-500" : "bg-gray-50 text-gray-400 hover:text-gray-600"}`}
                  >
                    ✨ Read-Along
                  </button>
                </div>
                ...
                <div className="max-h-[320px] overflow-y-auto p-4">
                  {!showReadAlong ? (
                    ...
                    /* ── Read-along plain text view ── */
                    <div>
                      {/* tiny legend */}
                      {(isSpeaking || isPaused) && (
                        <div className="flex items-center gap-2 mb-3 p-2 rounded-xl bg-violet-50 border border-violet-100">
                          <span
                            style={{
                              background: "linear-gradient(120deg,#a78bfa 0%,#60a5fa 100%)",
                              borderRadius: "4px",
                          ...
                          <span className="text-xs text-violet-600 font-semibold">= currently being spoken</span>
                        </div>
                      )}

                      <ReadAloudText
                        plainText={plainForDisplay}
